...
// TODO: memoize
const getSubmissionChainSymbolFromOutputTxType = curry((_outputTxType, _bridgeType) => {
...
const getBridgeSideForSymbol = curry((_bridgeType, _symbol) =>
  ...
  .then(([_hostSymbol, _nativeSymbol]) => {
    if (_hostSymbol === _symbol)
      return SIDE_HOST
     else if (_nativeSymbol === _symbol)
      return SIDE_NATIVE
     else
      return Promise.reject(new Error(`${ERROR_INVALID_SYMBOL_FOR_BRIDGE_TYPE} - ${_symbol}`))
  })
)
...
const getBridgeSideFromV2BridgeType = _bridgeName =>
  getBridgeSideForSymbol(_bridgeName, 'int')
    .then(_side => {
      switch(_side) {
        case SIDE_HOST:
          return SIDE_NATIVE
