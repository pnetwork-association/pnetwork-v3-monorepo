...
const defineMessageEventHandler = _ws => 
  new Promise(resolve => _ws.on('message', resolve))
    .then(_resp => logger.debug("Websocket 'message' event handler defined") || _resp)
  ...
  new Promise((resolve, reject) => 
    _ws.send(_data, _err => 
        _err instanceof Error 
          ? reject(_err) 
          : logger.trace(`Websocket data sent: ${_data}`) || 
            resolve(_ws)
      )
    )
...
const webSocketFetch = curry((_endpoint, _body, _timeout = 500) =>
  getWebSocketConnection(_endpoint)
    .then(_ws => webSocketSend(_ws, _body, _timeout))
)
