...
const deleteReportWithNonce = curry((_bridgeSide, _bridgeType, _legacy, _collection, _nonce) =>
  getReportIdFromNonce(_bridgeSide, _bridgeType, _legacy, _nonce)
    .then(deleteReport(_collection))
)
...
const setLastProcessedBlock = curry((_bridgeSide, _bridgeType, _collection, _blockNum) =>
  getChainSymbolFromBridgeType(_bridgeSide, _bridgeType)
    .then(getLastProcessedBlockReportId)
    .then(updateReport(_collection, { $set: { [ REPORTS_KEY_BLOCK_NUM ]: _blockNum } }))
)
...
const getReport = curry((_bridgeSide, _bridgeType, _legacy, _collection, _nonce) =>
  getReportIdFromNonce(_bridgeSide, _bridgeType, _legacy, _nonce)
    .then(findReportById(_collection))
)
...
const setFieldInReportToValue = curry((_field, _fieldType, _bridgeSide, _bridgeType, _legacy, _collection, _nonce, _value) =>
...
const updateReportWithNonce = curry((_bridgeSide, _bridgeType, _legacy, _collection, _nonce, _operations) =>
  getReportIdFromNonce(_bridgeSide, _bridgeType, _legacy, _nonce)
    .then(updateReport(_collection, _operations))
)
...
const approveReport = curry((_bridgeSide, _bridgeType, _legacy, _collection, _nonce, _txHash) =>
  getEmptyMongoPipeline()
    ...
    .then(updateReportWithNonce(_bridgeSide, _bridgeType, _legacy, _collection, _nonce))
