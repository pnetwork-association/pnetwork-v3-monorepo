...
const getClient = _url =>
  createConnection(_url)
    .catch(_err =>
      logger.error(ERROR_DB_CLIENT) || Promise.reject(_err)
    )
...
const getDatabase = memoizeWith(
  ...
  (_url, _databaseName) => 
    getClient(_url)
      .then(_client => _client.db(_databaseName))
)
...
const renameField = curry((_collection, _actualName, _newName, _id) =>
  _collection.updateOne({ _id: _id }, { $rename: { [_actualName ]: _newName } })
    .then(_output => logger.info(`Updated ${_output.modifiedCount} documents!`))
)

const renameReportsField = curry((_collection, _actualName, _newName, _idRegExp) =>
  _collection.updateMany({ _id: _idRegExp }, { $rename: { [ _actualName ]: _newName }})
    .then(_output => logger.info(`Updated ${_output.modifiedCount} documents!`))
)

const closeConnection = _dbUrl => // i.e. mongodb://127.0.0.1:27017
  getClient(_dbUrl)
