...
const rejectAfterXMilliseconds = _milliseconds =>
  new Promise((resolve, reject) => setTimeout(reject, _milliseconds, new Error(ERROR_TIMEOUT)))
...
const sleepThenReturnArg = R.curry((_milliseconds, _resolvedValue) =>
  R.isNil(_resolvedValue)
    ? Promise.reject(
        new Error(
...
const executePromiseWithRetries = R.curry((_options, _promiseFxn, _args = []) => {
  ...
  const attemptsExhausted = _attemptNum =>
    _attemptNum >= _options[FIELD_MAX_ATTEMPTS] || _attemptNum >= MAX_ATTEMPTS_CAP
  ...
  const retry = (_mainFlowHandler, _attemptNum) =>
    ...
    sleepForXMilliseconds(_options[FIELD_SLEEP_TIME]).then(() => _mainFlowHandler(_attemptNum + 1))

  const mainFlowNeverRetry = (_mainFlowHandler, _attemptNum, _err) => {
    if (attemptsExhausted(_attemptNum)) return rejectWithAttemptsExhaustedError(_err)
  ...
  const mainFlowAlwaysRetry = (_mainFlowHandler, _attemptNum, _err) => {
    if (matchStringInsideListSync(retryingExceptions, _err.message)) return rejectImmediately(_err)
    else if (attemptsExhausted(_attemptNum)) return rejectWithAttemptsExhaustedError(_err)
    else return retry(_mainFlowHandler, _attemptNum)
  }

  const mainFlowErrorHandler = R.curry((_mainFlowHandler, _attemptNum, _mode, _err) => {
    ...
    else return Promise.reject(new Error(`${ERROR_INVALID_RETRYING_MODE} (${_mode})`))
  })

  const mainFlow = (_attemptNum = 1) => {
    if (_attemptNum > 2) logger.info(`Trying ${promiseFxnName} attempt #${_attemptNum}...`)
    else logger.trace(`Trying ${promiseFxnName} attempt #${_attemptNum}...`)

    return _promiseFxn(..._args).catch(mainFlowErrorHandler(mainFlow, _attemptNum, mode))
  }

  const getMessageOrDefault = _object =>
    R.isNil(_object[FIELD_LOG_MSG])
...
const racePromise = (_milliseconds, _promiseFxn, _promiseFxnArgs = []) => {
  ...
  const _rejectAfterX = _x =>
    new Promise(
      (_, reject) => (timeoutReference = setTimeout(reject, _x, new Error(ERROR_TIMEOUT)))
    )

  return Promise.race([_promiseFxn(..._promiseFxnArgs), _rejectAfterX(_milliseconds)]).finally(
    () => {
