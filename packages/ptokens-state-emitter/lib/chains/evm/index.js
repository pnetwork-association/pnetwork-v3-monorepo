...
const addSyncStateToStatus = R.curry(
  (_status, _networkId, _block) =>
    logger.debug(`Status: Adding block ${_block.number} data to sync state...`) || {
      [_networkId]: {
        [constants.statusObject.KEY_LATEST_BLOCK_HASH]: _block.hash,
        [constants.statusObject.KEY_LATEST_BLOCK_NUMBER]: _block.number,
        [constants.statusObject.KEY_LATEST_BLOCK_TS]: _block.timestamp,
      },
    }
)
