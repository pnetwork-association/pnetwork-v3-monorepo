...
const getEthersProvider = R.memoizeWith(R.identity, _url => ethers.getDefaultProvider(_url))

const getEventFragment = _eventName => ethers.utils.EventFragment.from(_eventName)

const getInterfaceFromEvent = _eventName =>
  Promise.resolve(getEventFragment(_eventName))
    .then(_fragment => new ethers.utils.Interface([_fragment]))
...
const getFilterObject = (_eventName, _tokenContract) =>
  Promise.resolve(getEventFragment(_eventName))
    .then(_frag => ({
      ...
      topics: [
        ethers.utils.id(_frag.format())
      ]
    }))

const addLogInfo = R.curry((_log, _obj) =>
  Promise.resolve(_obj)
    .then(R.assoc('originatingTxHash', _log.transactionHash))
)
...
const listenFromFilter = (_filter, _interface, _callback) =>
  getEthersProvider(getConfiguration()['provider-url'])
    .on(_filter, processEventLog(_interface, _callback))
...
const listenForEvmEvent = (_eventName, _tokenContract, _callback) =>
  Promise.all([
    ...
    .then(([_filter, _interface]) => listenFromFilter(_filter, _interface, _callback))
