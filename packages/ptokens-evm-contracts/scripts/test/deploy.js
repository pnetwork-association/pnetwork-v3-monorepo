...
const main = async () => {
  ...
  const token = await StandardToken.deploy('Token', 'TKN', ethers.utils.parseEther('100000000'))
  ...
  const pToken = await deployPToken(await token.name(), await token.symbol(), await token.decimals(), token.address, PNETWORK_NETWORK_IDS.sepolia, {
    pFactory
  })
