...
const checkTokenBalanceIsSufficient = (
  _amount,
  _tokenOwnerAddress,
  _tokenAddress,
  _signer
  ...
  console.info(
    `✔ Checking spender ${_tokenOwnerAddress} has sufficient balance of token ${_tokenAddress}...`
  ) ||
  logic
    .racePromise(
    ...
    .then(_tokenBalance => {
      if (_tokenBalance.lt(BigNumber.from(_amount))) {
        return Promise.reject(
          new Error(
            `Insufficient token balance to peg in! Got ${_tokenBalance}, need ${_amount}!`
          )
        )
      } else {
