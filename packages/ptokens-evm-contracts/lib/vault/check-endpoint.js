const checkEndpoint = _provider => {
  ...
  const rejectAfterX = _x =>
    ...
    new Promise(
      (resolve, reject) =>
        (timeoutReference = setTimeout(reject, _x, new Error('Timed out!')))
    )
  ...
  return Promise.race([
    _provider.send('eth_blockNumber', []),
    rejectAfterX(TIMEOUT_TIME),
  ])
    .then(_blockNumberHex => parseInt(_blockNumberHex, 'hex'))
    .then(
      _blockNumber =>
        console.info(`✔ Endpoint working! Block height: ${_blockNumber}`) ||
        _provider
    )
