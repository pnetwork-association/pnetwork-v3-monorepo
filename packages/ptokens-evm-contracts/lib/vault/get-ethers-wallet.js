const { prop, curry, identity, memoizeWith } = require('ramda')
...
const getEthersWallet = memoizeWith(
  identity,
  _provider =>
    ...
    getGpgEncryptedPrivateKey()
      ...
      .then(
        _wallet =>
          console.info(
            `✔ Ethers wallet retrieved for address: ${prop(
              'address',
              _wallet
            )}!`
          ) || _wallet
      ...
      .catch(_err =>
        Promise.reject(
          new Error(`Error getting wallet from private key: ${_err.message}`)
        )
      )
)
