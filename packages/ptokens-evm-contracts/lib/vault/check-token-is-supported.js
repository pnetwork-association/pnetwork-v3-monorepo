...
const checkTokenIsSupportedInVault = (
  _vaultAddress,
  _tokenAddress,
  _provider
) =>
  console.info(
    `✔ Checking token @ ${_tokenAddress} is supported in vault @ ${_vaultAddress}...`
  ) ||
  logic
    .racePromise(
    ...
    .then(_tokenIsSupported => {
      if (_tokenIsSupported) {
        ...
        return Promise.reject(
          new Error(
            `Token @ ${_tokenAddress} is NOT supported in vault contract @ ${_vaultAddress}!`
          )
        )
      }
    })
