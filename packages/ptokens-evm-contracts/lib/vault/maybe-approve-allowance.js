...
const approveAllowance = (_amount, _vaultAddress, _tokenAddress, _signer) =>
  console.info(
    `✔ Approving vault to spend ${_amount} tokens, please wait for mining...`
  ) ||
...
const maybeApproveAllowance = (
  ...
  console.info(
    `✔ Checking spender ${_vaultAddress} has sufficient allowance for token ${_tokenAddress}...`
  ) ||
  logic
    .racePromise(
