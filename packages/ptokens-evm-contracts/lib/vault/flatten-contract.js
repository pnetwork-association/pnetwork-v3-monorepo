...
const { remove, identity, memoizeWith } = require('ramda')
...
const getFlattenCommand = _ =>
  ...
  `truffle-flattener ${path.resolve(
    __dirname,
    '../contracts/Erc20Vault.sol'
...
const removeAllButOneSPDXLicense = _ => {
  const removeLineWithSPDXLicenseRecursively = _lines => {
    ...
    return _lines.filter(predicateFxn).length > 1
      ? removeLineWithSPDXLicenseRecursively(
          remove(_lines.findIndex(predicateFxn), 1, _lines)
        )
      : _lines
  }
...
const flattenContract = __ =>
  ...
  executeCommand(getFlattenCommand())
    ...
    .then(_ =>
      console.info(
        `✔ Contract flattened! You can find it here:\n${getFullPathToFlattenedFile()}`
      )
    )
