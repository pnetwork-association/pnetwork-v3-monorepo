...
const getSuggestedFees = _provider =>
  console.info('✔ Getting suggeted fees...') || _provider.getFeeData()

const convertToGwei = _bigNumber => ethers.utils.formatUnits(_bigNumber, 'gwei')
...
const parseSuggestedFeesWithEIP1559Enabled = _suggestedFees =>
  console.info(
    '✔ Parsing suggested fees (EIP1559 is enabled on this chain!)...'
  ) || {
    maxFeePerGas: formatForPrinting(_suggestedFees.maxFeePerGas),
    maxPriorityFeePerGas: formatForPrinting(
      _suggestedFees.maxPriorityFeePerGas
    ),
...
const parseSuggestedFeesWithEIP1559NotEnabled = _suggestedFees =>
  console.info(
    '✔ Parsing suggested fees (EIP1559 is NOT enabled on this chain!)...'
