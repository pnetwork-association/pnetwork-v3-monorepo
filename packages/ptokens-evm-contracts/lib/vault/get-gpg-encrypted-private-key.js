...
const stripNewLines = _s => _s.replace(/\r?\n|\r/g, '')
...
const checkIsHex = _hex =>
  ...
  maybeAddHexPrefix(_hex).then(_prefixedHex =>
...
const checkPrivateKeyFileExists = _pathToPrivateKey =>
  ...
  new Promise((resolve, reject) =>
    existsSync(_pathToPrivateKey)
      ...
      : reject(
          new Error(
            `GPG encrypted private key does NOT exist @ '${_pathToPrivateKey}'`
          )
        )
  )
...
const checkPrivateKey = _privateKey =>
  ...
  checkIsHex(_privateKey).then(_checkedPrivateKey => {
...
const maybeGetPrivateKeyFiles = () =>
  utils
    .listFilesInFolder(path.resolve(__dirname, '..'))
    .then(utils.applyRegExpToListOfStrings(PRIVATE_KEY_REGEXP))
    .then(
      cli.maybeAskUserToSelectOption(
        'Select the private key to use:',
        'None file private-key*.gpg found!'
      )
    )
