...
const pegIn = (
  ...
  getVaultContract(_vaultAddress)
    .then(_vaultContract =>
      Promise.all([_vaultContract, _vaultContract.signer.getAddress()])
    )
    .then(([_vaultContract, _tokenOwnerAddress]) =>
      Promise.all([
        _vaultContract,
        _tokenOwnerAddress,
        checkTokenIsSupportedInVault(
          _vaultAddress,
          _tokenAddress,
    ...
    .then(([_vaultContract, _tokenOwnerAddress]) =>
      Promise.all([
        _vaultContract,
        maybeApproveAllowance(
          _amount,
          _vaultAddress,
          _tokenOwnerAddress,
          _tokenAddress,
          _vaultContract.signer
        ),
      ])
    )
    .then(([_vaultContract]) =>
    ...
    .then(_receipt =>
      console.info('✔ Success! Transaction receipt:\n', _receipt)
    )
