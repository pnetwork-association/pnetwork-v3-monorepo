...
const getWalletBalance = _wallet =>
  getWalletAddress(_wallet)
    .then(_address =>
      _wallet.provider.send('eth_getBalance', [_address, 'latest'])
    )
...
const showWalletDetails = _ =>
  ...
  getEnvConfiguration()
    ...
    .then(_wallet =>
      Promise.all([getWalletAddress(_wallet), getWalletBalance(_wallet)])
    )
    .then(([_address, _balance]) => {
      console.info('✔ Wallet address:', _address)
      console.info(
        `✔ Wallet balance: ${_balance} (${ethers.utils.formatUnits(
          _balance,
          'ether'
        )} ETH)`
      )
