...
const getVaultContractFactory = _wallet =>
  Promise.all([getVaultAbi(), getVaultBytecode()]).then(([_abi, _bytecode]) =>
    getContractFactory(_abi, _bytecode, _wallet)
  )

const getUnwrapperContractFactory = _wallet =>
  Promise.all([getUnwrapperAbi(), getUnwrapperBytecode()]).then(
    ([_abi, _bytecode]) => getContractFactory(_abi, _bytecode, _wallet)
  )
