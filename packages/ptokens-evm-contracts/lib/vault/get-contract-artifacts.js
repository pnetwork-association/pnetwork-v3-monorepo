...
const VAULT_RELATIVE_PATH =
  '../artifacts/contracts/Erc20Vault.sol/Erc20Vault.json'
const UNWRAPPER_RELATIVE_PATH =
  '../artifacts/contracts/weth-unwrapper/WEthUnwrapper.sol/WEthUnwrapper.json'

const getFullPathToArtifact = memoizeWith(identity, _relativePath =>
  path.resolve(__dirname, _relativePath)
)

const getArtifact = memoizeWith(
  identity,
  _path =>
    new Promise((resolve, reject) => {
      ...
      exists
        ...
        : reject(
            new Error(
              'Artifact does not exist! Run `npx hardhat compile` to compile contracts!'
            )
          )
    })
)
...
const parseVaultArtifact = _key =>
  getArtifact(getFullPathToArtifact(VAULT_RELATIVE_PATH)).then(_artifact =>
    getKeyFromObj(ARTIFACT_OBJECT_NAME, _artifact, _key)
  )

const parseUnwrapperArtifact = _key =>
  getArtifact(getFullPathToArtifact(UNWRAPPER_RELATIVE_PATH)).then(_artifact =>
    getKeyFromObj(ARTIFACT_OBJECT_NAME, _artifact, _key)
  )
