...
const assertPegInEvent = (
  ...
  assert.strictEqual(
    _pegInEvent.args._tokenAmount.toString(),
    `${_tokenAmount}`
  )
}
...
const deployNonUpgradeableContract = (_contractPath, _deployArgs = []) =>
  ethers
    ...
    .then(_contract =>
      Promise.all([_contract, _contract.deployTransaction.wait()])
    )
...
const getEventSignatureFromEventFragment = _eventFragment =>
  ethers.utils.keccak256(
    ethers.utils.toUtf8Bytes(
      `${_eventFragment.name}(${_eventFragment.inputs
        .map(prop('type'))
        .reduce(
          (_acc, _type, _i) => `${_acc}${_i === 0 ? '' : ','}${_type}`,
          ''
        )})`
    )
  )
