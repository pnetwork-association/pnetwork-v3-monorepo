...
describe('Erc20Vault Tests', () => {
  ...
  const VAULT_PATH = 'contracts/vault/Erc20Vault.sol:Erc20Vault'
  const UNWRAPPER_PATH =
    'contracts/vault/weth-unwrapper/WEthUnwrapper.sol:WEthUnwrapper'
  const NON_PNETWORK_ERROR = 'Caller must be PNETWORK address!'
  const ERC777_CONTRACT_PATH =
    'contracts/vault/test-contracts/Erc777Token.sol:Erc777Token'
  ...
  beforeEach(async () => {
    ...
    WETH_CONTRACT = await deployNonUpgradeableContract(
      'contracts/vault/wEth/WETH.sol:WETH'
    )
    WETH_ADDRESS = prop(ADDRESS_PROP, WETH_CONTRACT)
    VAULT_CONTRACT = await deployUpgradeableContract(VAULT_PATH, [
      WETH_ADDRESS,
      [],
      ORIGIN_CHAIN_ID,
    ])
    VAULT_ADDRESS = prop(ADDRESS_PROP, VAULT_CONTRACT)
    UNWRAPPER_CONTRACT = await deployNonUpgradeableContract(UNWRAPPER_PATH, [
      WETH_ADDRESS,
    ])
    UNWRAPPER_ADDRESS = prop(ADDRESS_PROP, UNWRAPPER_CONTRACT)
    ERC20_TOKEN_CONTRACT = await deployNonUpgradeableContract(
      'contracts/vault/test-contracts/Erc20Token.sol:Erc20Token'
    )
    ERC20_TOKEN_ADDRESS = prop(ADDRESS_PROP, ERC20_TOKEN_CONTRACT)
    NON_OWNED_VAULT_CONTRACT = VAULT_CONTRACT.connect(NON_PNETWORK)
    await ERC20_TOKEN_CONTRACT.transfer(
      TOKEN_HOLDER_ADDRESS,
      TOKEN_HOLDER_BALANCE
    )
    const tokenHolderBalance = await ERC20_TOKEN_CONTRACT.balanceOf(
      TOKEN_HOLDER_ADDRESS
    )
    ...
    await ERC777_CONTRACT.send(
      TOKEN_HOLDER_ADDRESS,
      TOKEN_AMOUNT,
      EMPTY_USER_DATA
    )
    await ERC777_CONTRACT.connect(TOKEN_HOLDER).approve(
      VAULT_ADDRESS,
      TOKEN_AMOUNT
    )
  })
  ...
  describe('Initalizer Tests', () => {
    it('Token addresses sent to constructor should be supported', async () => {
      ...
      const newVault = await deployUpgradeableContract(VAULT_PATH, [
        WETH_ADDRESS,
        addresses,
        ORIGIN_CHAIN_ID,
      ])
      addresses.map(async _address => {
  ...
  describe('Token Approval Tests', () => {
    ...
    it('PNETWORK_ADDRESS can add appoved token address', async () => {
      tokenIsSupported = await VAULT_CONTRACT.isTokenSupported(
        ERC20_TOKEN_ADDRESS
      )
      assert(!tokenIsSupported)
    ...
    it('NON_PNETWORK_ADDRESS cannot remove appoved token address', async () => {
      tokenIsSupported = await NON_OWNED_VAULT_CONTRACT.isTokenSupported(
        ERC20_TOKEN_ADDRESS
      )
      assert(!tokenIsSupported)
      await VAULT_CONTRACT.addSupportedToken(ERC20_TOKEN_ADDRESS)
  ...
  describe('Peg In Tests', () => {
    ...
    it('Should peg in if token & destination are supported and sufficient allowance approved', async () => {
      await addTokenSupport(VAULT_CONTRACT, ERC20_TOKEN_ADDRESS)
      await giveAllowance(
        ERC20_TOKEN_CONTRACT.connect(TOKEN_HOLDER),
        VAULT_ADDRESS,
        TOKEN_AMOUNT
      )
      const tokenHolderBalanceBeforePegIn =
        await ERC20_TOKEN_CONTRACT.balanceOf(TOKEN_HOLDER_ADDRESS)
      const vaultTokenBalanceBefore = await ERC20_TOKEN_CONTRACT.balanceOf(
        VAULT_ADDRESS
      )
      ...
      const vaultTokenBalanceAfter = await ERC20_TOKEN_CONTRACT.balanceOf(
        VAULT_ADDRESS
      )
      const tokenHolderBalanceAfterPegIn = await ERC20_TOKEN_CONTRACT.balanceOf(
        TOKEN_HOLDER_ADDRESS
      )
      assert(
        vaultTokenBalanceAfter.eq(vaultTokenBalanceBefore.add(TOKEN_AMOUNT))
      )
      assert(
        tokenHolderBalanceAfterPegIn.eq(
          tokenHolderBalanceBeforePegIn.sub(TOKEN_AMOUNT)
        )
      )
    })
    ...
    it('Should peg in with user data', async () => {
      ...
      const tx = await VAULT_CONTRACT.connect(TOKEN_HOLDER)[
  ...
  describe('ERC777 Peg In Tests', () => {
    ...
    it('Should automatically peg in on ERC777 send', async () => {
      const eventFragment = find(
        x => x.name === 'PegIn' && x.type === 'event',
        VAULT_CONTRACT.interface.fragments
      )
      ...
      const tx = await ERC777_CONTRACT.connect(TOKEN_HOLDER).send(
        VAULT_ADDRESS,
        TOKEN_AMOUNT,
        userData
      )
      const receipt = await tx.wait()
      const event = find(
        e => e.topics[0] === eventSignature,
        values(receipt.events)
      )
      // eslint-disable-next-line eqeqeq
      assert(event != null, 'PegIn event not found')
      const decodedEvent = ABI_CODEC.decode(
        eventFragment.inputs.map(prop('type')),
        event.data
      )
    ...
    it('Should fail to automatically peg-in if encoded user data is incorrect', async () => {
      const tag = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(PEG_IN_TAG))
      const malformedUserData = ABI_CODEC.encode(
        ['bytes32', 'bytes4'],
        [tag, DESTINATION_CHAIN_ID]
      )
      await ERC777_CONTRACT.send(
        TOKEN_HOLDER_ADDRESS,
        TOKEN_AMOUNT,
      ...
      try {
        await ERC777_CONTRACT.connect(TOKEN_HOLDER).send(
          VAULT_ADDRESS,
          TOKEN_AMOUNT,
          malformedUserData
        )
    ...
    it('Should fail to automatically peg-in if encoded peg-in tag is incorrect', async () => {
      const tag = ethers.utils.keccak256(
        ethers.utils.toUtf8Bytes('Incorrect peg-in tag')
      )
      const userData = ABI_CODEC.encode(
        ['bytes32', 'string', 'bytes4'],
        [tag, DESTINATION_ADDRESS, DESTINATION_CHAIN_ID]
  ...
  describe('Peg Out Tests', () => {
    ...
    it('PNETWORK_ADDRESS can peg out with sufficient balance', async () => {
      ...
      const tokenHolderBalanceBeforePegOut =
        await ERC20_TOKEN_CONTRACT.balanceOf(TOKEN_HOLDER_ADDRESS)
      await pegOut(
        VAULT_CONTRACT,
        TOKEN_HOLDER_ADDRESS,
      ...
      const tokenHolderBalanceAfterPegOut =
        await ERC20_TOKEN_CONTRACT.balanceOf(TOKEN_HOLDER_ADDRESS)
      assert(
        tokenHolderBalanceAfterPegOut.eq(
          tokenHolderBalanceBeforePegOut.add(TOKEN_AMOUNT)
        )
      )
    })
    ...
    it('Can peg out with user data', async () => {
      ...
      await giveAllowance(
        ERC777_CONTRACT.connect(TOKEN_HOLDER),
        VAULT_ADDRESS,
        TOKEN_AMOUNT
      )
      await pegInWithoutUserData(
      ...
      const tokenHolderBalanceBeforePegOut = await ERC777_CONTRACT.balanceOf(
        TOKEN_HOLDER_ADDRESS
      )
      await pegOut(
        VAULT_CONTRACT,
      ...
      const tokenHolderBalanceAfterPegOut = await ERC777_CONTRACT.balanceOf(
        TOKEN_HOLDER_ADDRESS
      )
      assert(
        tokenHolderBalanceAfterPegOut.eq(
    ...
    it('Pegging out to ERC777 recipient with user data will call tokens received hook', async () => {
      const userData = '0xdecaff'
      const ERC777_RECIPIENT_PATH =
        'contracts/vault/test-contracts/Erc777Recipient.sol:Erc777Recipient'
      await giveAllowance(
        ERC777_CONTRACT.connect(TOKEN_HOLDER),
        VAULT_ADDRESS,
        TOKEN_AMOUNT
      )
      const ERC777_RECIPIENT_CONTRACT = await deployNonUpgradeableContract(
        ERC777_RECIPIENT_PATH
      )
      const ERC777_RECIPIENT_ADDRESS = prop(
        ADDRESS_PROP,
      ...
      const recipientBalanceBefore = await ERC777_CONTRACT.balanceOf(
      ...
      const recipientBalanceAfter = await ERC777_CONTRACT.balanceOf(
        ERC777_RECIPIENT_ADDRESS
      )
      assert(recipientBalanceAfter.eq(recipientBalanceBefore.add(TOKEN_AMOUNT)))
      assert(await ERC777_RECIPIENT_CONTRACT.tokenReceivedCalled())
      const userDataFromRecipient =
        await ERC777_RECIPIENT_CONTRACT.tokensReceivedData()
  ...
  describe('wETH Tests', () => {
    ...
    describe('Peg In wETH Tests', () => {
      it('Should peg in wETH', async () => {
        const tx = await VAULT_CONTRACT.connect(TOKEN_HOLDER)[
          'pegInEth(string,bytes4)'
        ](DESTINATION_ADDRESS, DESTINATION_CHAIN_ID, { value: TOKEN_AMOUNT })
        const receipt = await tx.wait()
        assertPegInEvent(
          await getPegInEventFromReceipt(receipt),
          WETH_ADDRESS,
        ...
        const vaultWethBalanceAfter = await WETH_CONTRACT.balanceOf(
          VAULT_ADDRESS
        )
        const vaultEthBalanceAfter = await ethers.provider.getBalance(
          VAULT_ADDRESS
        )
      ...
      it('Should peg in wETH with user data', async () => {
        const tx = await VAULT_CONTRACT.connect(TOKEN_HOLDER)[
          'pegInEth(string,bytes4,bytes)'
        ](DESTINATION_ADDRESS, DESTINATION_CHAIN_ID, USER_DATA, {
          value: TOKEN_AMOUNT,
        })
        const receipt = await tx.wait()
        assertPegInEvent(
    ...
    describe('Peg Out wETH Tests', () => {
      it('Should peg out wETH without user data', async () => {
        ...
        const tokenHolderEthBalanceBefore = await ethers.provider.getBalance(
          TOKEN_HOLDER_ADDRESS
        )
        await pegOut(
          VAULT_CONTRACT,
        ...
        const expectedTokenHolderEthBalance = BigNumber.from(
          tokenHolderEthBalanceBefore
        ).add(TOKEN_AMOUNT)
        const tokenHolderEthBalance = await ethers.provider.getBalance(
          TOKEN_HOLDER_ADDRESS
        )
        assert(vaultEthBalanceAfter.eq(0))
      ...
      it('Should peg out wETH with user data', async () => {
        ...
        const vaultEthBalanceAfterPegOut = await ethers.provider.getBalance(
          VAULT_ADDRESS
        )
        const expectedTokenHolderEthBalance = BigNumber.from(
          tokenHolderEthBalanceBefore
        ).add(TOKEN_AMOUNT)
        const tokenHolderEthBalanceAfter = await ethers.provider.getBalance(
          TOKEN_HOLDER_ADDRESS
        )
        assert(vaultWethBalanceAfter.eq(0))
      ...
      it('Should peg out wETH to smart-contract w/ expensive fallback function', async () => {
        ...
        const PATH =
          'contracts/vault/test-contracts/ContractWithExpensiveFallbackFunction.sol:ContractWithExpensiveFallbackFunction'
        const expensiveFallbackContract = await deployNonUpgradeableContract(
          PATH
        )
        const expensiveFallbackContractAddress = prop(
          ADDRESS_PROP,
          expensiveFallbackContract
        )
        const expensiveContractEthBalanceBeforePegout =
          await ethers.provider.getBalance(expensiveFallbackContractAddress)
        ...
        assert(vaultWethBalanceAfter.eq(0))
        const expensiveContractEthBalanceAfterPegout =
          await ethers.provider.getBalance(expensiveFallbackContractAddress)
      ...
      it('Should be able to peg out wETH with user data to a smart-contract', async () => {
        ...
        const userDataFromRecipient =
          await ERC777_RECIPIENT_CONTRACT.fallbackFunctionCallData()
        assert(
          recipientBalanceAfter.eq(recipientBalanceBefore.add(TOKEN_AMOUNT))
        )
        assert.strictEqual(userDataFromRecipient, userData)
      })
      ...
      it('Pegging out wETH Should not be susceptible to re-entrancy attack', async () => {
        const calldata = new ethers.utils.Interface([
          'function attempReEntrancyAttack()',
        ]).encodeFunctionData('attempReEntrancyAttack', [])
        const CONTRACT_PATH =
          'contracts/vault/test-contracts/ContractWithReEntrancyAttack.sol:ContractWithReEntrancyAttack'
        const reEntrancyAttackContract = await deployNonUpgradeableContract(
          CONTRACT_PATH
        )
        const reEntrancyAttackContractAddress = prop(
          ADDRESS_PROP,
          reEntrancyAttackContract
        )
        const reEntrancyAttackContractEthBalanceBeforePegout =
          await ethers.provider.getBalance(reEntrancyAttackContractAddress)
