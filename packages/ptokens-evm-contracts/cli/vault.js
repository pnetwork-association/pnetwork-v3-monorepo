...
const { pegIn } = require('../lib/vault/peg-in')
const { setPNetwork } = require('../lib/vault/set-pnetwork')
const { getPNetwork } = require('../lib/vault/get-pnetwork')
const { deployVault } = require('../lib/vault/deploy-vault')
const { verifyVault } = require('../lib/vault/verify-vault')
const { getWEthAddress } = require('../lib/vault/get-weth-address')
const { verifyUnwrapperContract } = require('../lib/vault/verify-unwrapper')
const { isTokenSupported } = require('../lib/vault/is-token-supported')
const { showWalletDetails } = require('../lib/vault/show-wallet-details')
const { addSupportedToken } = require('../lib/vault/add-supported-token')
const { showSuggestedFees } = require('../lib/vault/show-suggested-fees')
const { getSupportedTokens } = require('../lib/vault/get-supported-tokens')
const { deployUnwrapperContract } = require('../lib/vault/deploy-unwrapper')
const { getEncodedInitArgs } = require('../lib/vault/get-encoded-init-args')
const {
  setWEthUnwrapperAddress,
} = require('../lib/vault/set-weth-unwrapper-address')
const {
  showExistingContractAddresses,
} = require('../lib/vault/show-existing-contract-addresses')
...
const main = _ => {
  ...
  if (CLI_ARGS[FLATTEN_CONTRACT_CMD]) {
    return Promise.resolve(
      console.info(
        '✘ `truffle-flattener` does not work w/ `unchecked` code blocks, so we cannot do this yet!'
      )
    )
    ...
    return setWEthUnwrapperAddress(
      CLI_ARGS[DEPLOYED_ADDRESS_ARG],
      CLI_ARGS[ETH_ADDRESS_ARG]
    )
