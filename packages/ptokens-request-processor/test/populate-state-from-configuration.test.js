  ...
  describe('getInitialStateFromConfiguration', () => {
    it('Should populate the state from the configuration', async () => {
      const config = {
        ...
        [constants.config.KEY_LOOP_SLEEP_TIME]: 5000,
        [constants.config.KEY_TX_TIMEOUT]: 1500,
        [constants.config.KEY_BALANCE_THRESHOLD]: '1',
      }
      expect(
      ...
      assert.deepStrictEqual(ret, {
        ...
        [constants.state.KEY_LOOP_SLEEP_TIME]: 5000,
        [constants.state.KEY_TX_TIMEOUT]: 1500,
        [constants.state.KEY_BALANCE_THRESHOLD]: '1',
      })
    })
    ...
    it('Should assign default values for unspecified configurations', async () => {
      const config = {
        [constants.config.KEY_CHAIN_NAME]: 'Ethereum Mainnet',
        [constants.config.KEY_CHAIN_TYPE]: 'EVM',
        [constants.config.KEY_NETWORK_ID]: '0x005fe7f9',
      ...
      assert.deepStrictEqual(ret, {
        ...
        [constants.state.KEY_BALANCE_THRESHOLD]: DEFAULT_BALANCE_THRESHOLD,
      })
    })
  })
})
